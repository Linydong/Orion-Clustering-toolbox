...
axes(handles.axes7);
images(handles.axes7,'Logos/icon31.jpg');
...
switch method
    case 1
        ...
        set(handles.text29,'string','K_means');
    case 2
    ...
    case 3
        ...
        set(handles.text29,'string','DBSCAN');
end
...
if handles.method == 3
    ...
    if Eps <= 0 || Minpts~=fix(Minpts) || Minpts <= 0 
        choice = warndialog('Minpts 必须是正整数, Eps必须是正数 ','提醒','modal');
    else
        if Eps >= 1 || Minpts >= 10
            choice = choosedialog('Minpts或（和）Eps取值不适合时，聚类效果可能不佳，是否继续?','提示','modal');
        end
    ...
    if K <= 0 || K~=fix(K)
        warndialog('请输入正整数作为参数 K ','提醒','modal');
        choice = 'NO';
    else
        if K==1 || K>=10
